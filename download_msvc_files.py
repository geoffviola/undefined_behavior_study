...
def parse_args():
    ...
    parser.add_argument('--text_url', type=str,
                        default="https://ci.appveyor.com/api/buildjobs/hlj6seye4ijtmia4/log",
                        help='msvc text file')
...
def check_file_open(file_writing, file_being_written, no_timestamp, args):
    ...
    if no_timestamp == b"MSVC RelWithDebInfo Compiler Warnings Start\r\n":
        ...
        file_being_written = open(args.output_static_analysis_relwithdebinfo, 'w')
...
def write_file(file_writing, file_being_written, no_timestamp, args):
    file_writing, file_being_written = check_close_file(file_writing, file_being_written, no_timestamp)
    if file_writing:
        file_being_written.write(no_timestamp.decode("utf-8"))
    file_writing, file_being_written = check_file_open(file_writing, file_being_written, no_timestamp, args)
    return (file_writing, file_being_written)
    ...
    with urllib.request.urlopen(args.text_url) as msvc_text:
       for l in msvc_text:
           if len(l) < 11:
               ...
               exit(1)
           no_timestamp = l[11:] 
           file_writing, file_being_written = write_file(file_writing, file_being_written, no_timestamp, args)
